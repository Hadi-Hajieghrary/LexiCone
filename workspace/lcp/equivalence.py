...
# ----------------------------------------------------------------------
# Algorithm 0: homogeneous-cone primary formulation (Section 9.1)
# ----------------------------------------------------------------------


@dataclass
class Algorithm0Inputs:
    """Inputs for Algorithm 0 (L_1 homogeneous-cone primary formulation).

    Per v10_2 Section 9.1, the homogeneous cone

    .. math::
        \\widehat{\\Omega}(p^\\star) = N_{\\overline{\\mathcal{P}}}^+(p^\\star)
            \\cap (\\mathbb{R}_{\\geq 0}^L \\times \\mathbb{R}_{> 0})

    contains points :math:`(w, \\beta)` with :math:`\\beta > 0` the performance
    coefficient. The Chebyshev centre is found on the simplex slice
    :math:`\\Sigma := \\{(w, \\beta) : \\beta + \\sum_i w_i = 1,\\ w_i \\geq
    \\underline w_i,\\ \\beta \\geq \\underline\\beta\\}`, then the
    unit-performance WS weights are recovered by projection
    :math:`\\bar w_i := w_i^\\sharp / \\beta^\\sharp`.

    Fields mirror :class:`WeightCalibrationInputs` except for the lower bounds:
    Algorithm 0 takes per-level lower bounds :math:`\\underline w_i > 0` and a
    single :math:`\\underline\\beta > 0`, with no upper bounds (the simplex
    slice is bounded by construction via the equality
    :math:`\\beta + \\sum_i w_i = 1`).
    """

    grad_J: np.ndarray
    active_rule_constraints: List[ActiveConstraint]
    active_phys_inequalities: List[np.ndarray]
    active_equalities: List[np.ndarray]
    n_levels: int
    w_lower: np.ndarray            # shape (L,), strictly positive
    beta_lower: float              # > 0
    tikhonov_reg: float = 0.0


@dataclass
class Algorithm0Result:
    """Output of Algorithm 0.

    ``w_dagger`` is the unit-performance projection :math:`\\bar w` of
    :math:`(w^\\sharp, \\beta^\\sharp)` (the simplex-slice Chebyshev centre)
    via :math:`\\bar w_i = w_i^\\sharp / \\beta^\\sharp`. ``w_sharp`` and
    ``beta_sharp`` are the un-projected simplex-slice coordinates, retained
    for diagnostics.

    ``r_sharp`` is the Chebyshev radius on the simplex slice (NOT in the
    projected unit-performance space); it certifies the minimum slack on the
    cone's facets and the simplex-slice lower bounds.
    """

    w_dagger: np.ndarray           # projected unit-performance weights, shape (L,)
    w_sharp: np.ndarray            # unprojected simplex-slice w, shape (L,)
    beta_sharp: float              # unprojected simplex-slice beta
    r_sharp: float                 # Chebyshev radius on the simplex slice
    alpha_at_optimum: np.ndarray   # subgradient bounds at boundary-binding (n_bdy,)
    lambda_at_optimum: np.ndarray  # hard-inequality multipliers (n_phys,)
    mu_at_optimum: np.ndarray      # equality multipliers (n_eq,)
    lp_status: str
    notes: str = ""


def algorithm_0_homogeneous(inputs: Algorithm0Inputs) -> Algorithm0Result:
    """Algorithm 0 of v10_2 Section 9.1: homogeneous-cone primary formulation.

    Solves the lifted Chebyshev LP on the simplex slice of the homogeneous
    cone :math:`\\widehat{\\Omega}(p^\\star)`, then projects to unit-performance
    WS weights via :math:`\\bar w_i = w_i^\\sharp / \\beta^\\sharp`.

    Variable layout in the LP::

        [w (L) | beta_perf (1) | alpha (n_bdy) | lambda (n_phys) | mu (n_eq) | r (1)]

    Equality constraints:

    * d-dim KKT stationarity at :math:`z_\\text{lex}^\\star` for the WS
      problem :math:`\\beta J + \\sum_i w_i V_i`:

      .. math::
          \\beta_\\text{perf} \\nabla J + \\sum_{(i,j) \\in S_\\text{bdy}} \\alpha_{i,j} \\nabla g_{i,j}
              + \\sum_{i \\in S_\\text{viol}} w_i \\nabla V_i
              + \\sum_{p} \\lambda_p \\nabla g_{\\text{phys},p}
              + \\sum_e \\mu_e \\nabla h_e = 0

    * 1-dim simplex slice :math:`\\beta_\\text{perf} + \\sum_i w_i = 1`.

    Inequality constraints (Chebyshev face slacks):

    * Per-level lower-bound face: :math:`w_i - \\underline w_i \\geq r`
    * Performance-coefficient lower-bound face: :math:`\\beta_\\text{perf} - \\underline\\beta \\geq r`
    * L1 subgradient lower-bound face: :math:`\\alpha_{i,j} \\geq r` (so 0 stays away)
    * L1 subgradient upper-bound face: :math:`w_i - \\alpha_{i,j} \\geq r`
    * Hard-inequality multiplier lower-bound face: :math:`\\lambda_p \\geq r`
    * :math:`r \\geq 0` (LP feasibility certificate)

    The objective maximises :math:`r`. The optimum :math:`(w^\\sharp,
    \\beta^\\sharp, r^\\sharp)` is then projected as
    :math:`\\bar w_i = w_i^\\sharp / \\beta^\\sharp` (Step 0.3).
    """
    L = inputs.n_levels
    d = inputs.grad_J.shape[0]
    w_lower = np.asarray(inputs.w_lower, dtype=np.float64).reshape(L)
    beta_lower = float(inputs.beta_lower)
    if np.any(w_lower <= 0):
        raise ValueError(f"w_lower must be strictly positive (got {w_lower})")
    if beta_lower <= 0:
        raise ValueError(f"beta_lower must be strictly positive (got {beta_lower})")
    # Simplex feasibility: sum of lower bounds plus beta_lower must be <= 1.
    if beta_lower + float(w_lower.sum()) >= 1.0:
        raise ValueError(
            f"infeasible simplex slice: beta_lower + sum(w_lower) "
            f"= {beta_lower + float(w_lower.sum())} must be < 1"
        )
    ...
    n_vars = L + 1 + n_bdy + n_phys + n_eq + 1
    w_slice = slice(0, L)
    beta_perf_idx = L
    alpha_slice = slice(L + 1, L + 1 + n_bdy)
    lambda_slice = slice(L + 1 + n_bdy, L + 1 + n_bdy + n_phys)
    mu_slice = slice(L + 1 + n_bdy + n_phys, L + 1 + n_bdy + n_phys + n_eq)
    r_idx = n_vars - 1

    # ---------- Equality constraints ----------
    # (a) d-dim KKT stationarity.
    # (b) 1-dim simplex slice: beta_perf + sum w_i = 1.
    A_eq = np.zeros((d + 1, n_vars))
    b_eq = np.zeros(d + 1)
    # beta_perf * grad_J
    A_eq[0:d, beta_perf_idx] = inputs.grad_J.astype(np.float64).reshape(d)
    # w_i * grad_V_i (aggregated from violated levels)
    grad_V_per_level = np.zeros((d, L))
    for c in viol:
        grad_V_per_level[:, c.level_index] += c.gradient
    A_eq[0:d, w_slice] = grad_V_per_level
    # alpha_{i,j} * grad_g_{i,j} at boundary-binding
    for j_b, c in enumerate(bdy):
        A_eq[0:d, L + 1 + j_b] = c.gradient
    # lambda_p * grad_g_phys_p
    for j_p, g in enumerate(inputs.active_phys_inequalities):
        A_eq[0:d, L + 1 + n_bdy + j_p] = np.asarray(g, dtype=np.float64).reshape(d)
    # mu_e * grad_h_e
    for j_e, h in enumerate(inputs.active_equalities):
        A_eq[0:d, L + 1 + n_bdy + n_phys + j_e] = np.asarray(h, dtype=np.float64).reshape(d)
    # Simplex equality.
    A_eq[d, w_slice] = 1.0
    A_eq[d, beta_perf_idx] = 1.0
    b_eq[d] = 1.0

    # ---------- Inequality constraints (Chebyshev slack form, A_ub x <= b_ub) ----------
    A_ub_rows: List[np.ndarray] = []
    b_ub: List[float] = []
    # w_i - w_lower_i >= r  <=>  -w_i + r <= -w_lower_i
    for i in range(L):
        row = np.zeros(n_vars)
        row[i] = -1.0
        row[r_idx] = 1.0
        A_ub_rows.append(row)
        b_ub.append(-w_lower[i])
    # beta_perf - beta_lower >= r  <=>  -beta_perf + r <= -beta_lower
    row = np.zeros(n_vars)
    row[beta_perf_idx] = -1.0
    row[r_idx] = 1.0
    A_ub_rows.append(row)
    b_ub.append(-beta_lower)
    # alpha_{i,j} >= r  <=>  -alpha + r <= 0  (lower L1-subdiff bound stays away)
    for j_b in range(n_bdy):
        row = np.zeros(n_vars)
        row[L + 1 + j_b] = -1.0
        row[r_idx] = 1.0
        A_ub_rows.append(row)
        b_ub.append(0.0)
    # w_i - alpha_{i,j} >= r  <=>  alpha - w_i + r <= 0  (upper L1-subdiff bound)
    for j_b, c in enumerate(bdy):
        i = c.level_index
        row = np.zeros(n_vars)
        row[L + 1 + j_b] = 1.0
        ...
        A_ub_rows.append(row)
        b_ub.append(0.0)
    # lambda_p >= r  <=>  -lambda + r <= 0
    for j_p in range(n_phys):
        row = np.zeros(n_vars)
        row[L + 1 + n_bdy + j_p] = -1.0
        row[r_idx] = 1.0
        A_ub_rows.append(row)
        b_ub.append(0.0)
    ...
    bounds: List[Tuple[Optional[float], Optional[float]]] = []
    for i in range(L):
        bounds.append((w_lower[i], 1.0))          # w_i in [w_lower_i, 1] (simplex)
    bounds.append((beta_lower, 1.0))              # beta_perf in [beta_lower, 1]
    for _ in range(n_bdy):
        bounds.append((0.0, None))                # alpha >= 0
    for _ in range(n_phys):
        bounds.append((0.0, None))                # lambda >= 0
    for _ in range(n_eq):
        bounds.append((None, None))               # mu free
    bounds.append((0.0, None))                    # r >= 0
    ...
    c_obj[r_idx] = -1.0
    if inputs.tikhonov_reg > 0:
        c_obj[alpha_slice] += inputs.tikhonov_reg
        c_obj[lambda_slice] += inputs.tikhonov_reg
    ...
    if not res.success:
        logger.warning(
            "Algorithm 0 LP failed: status=%s, message=%s; "
            "falling back to simplex centroid.",
            res.status, res.message,
        )
        # Fallback: uniform allocation on the simplex slice respecting lower bounds.
        free_mass = 1.0 - beta_lower - float(w_lower.sum())
        if free_mass <= 0:
            w_fallback = w_lower.copy()
            beta_fallback = beta_lower
        else:
            extra = free_mass / (L + 1)
            w_fallback = w_lower + extra
            beta_fallback = beta_lower + extra
        return Algorithm0Result(
            w_dagger=w_fallback / beta_fallback,
            w_sharp=w_fallback,
            beta_sharp=beta_fallback,
            r_sharp=0.0,
            alpha_at_optimum=np.zeros(n_bdy),
            lambda_at_optimum=np.zeros(n_phys),
            mu_at_optimum=np.zeros(n_eq),
            lp_status=str(res.message),
            notes="LP infeasible; falling back to simplex slice centroid",
        )

    x = res.x
    w_sharp = x[w_slice].copy()
    beta_sharp = float(x[beta_perf_idx])
    r_sharp = float(x[r_idx])
    if beta_sharp <= 0:
        raise RuntimeError(
            f"Algorithm 0 produced beta_sharp <= 0 ({beta_sharp}); LP solver bug?"
        )
    w_dagger = w_sharp / beta_sharp
    return Algorithm0Result(
        w_dagger=w_dagger,
        w_sharp=w_sharp,
        beta_sharp=beta_sharp,
        r_sharp=r_sharp,
        alpha_at_optimum=x[alpha_slice].copy() if n_bdy > 0 else np.zeros(0),
        lambda_at_optimum=x[lambda_slice].copy() if n_phys > 0 else np.zeros(0),
        mu_at_optimum=x[mu_slice].copy() if n_eq > 0 else np.zeros(0),
        lp_status=str(res.message),
    )
