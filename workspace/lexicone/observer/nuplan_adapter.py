...
def _map_to_snapshot(
    ...
    if include_lane_connectors:
        requested.append(SL.LANE_CONNECTOR)

    # NuPlanMap exposes some semantic layers only as rasters (DRIVABLE_AREA in
    # particular); querying them as map-objects raises. Filter to whatever the
    # map advertises so the adapter is portable across map versions.
    try:
