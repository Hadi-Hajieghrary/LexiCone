"""Smoke tests for the trajectory observer.

These tests use synthetic scenes (no nuplan-devkit) to verify:
- the observer can be constructed with the default 25-rule set;
...
def test_speed_limit_violation_triggers():
    ...
    obs.step(snap)
    by_id = {e.rule_id: e for e in obs.history[-1]}
    e = by_id["3r0"]
    assert e.applies
...
def test_speed_limit_not_applicable_without_lane_limit():
    obs = TrajectoryObserver()
    lane = straight_lane(speed_limit_mps=None)
    # We override the default lane by passing extra_lanes plus a custom map.
    snap = build_straight_road_scene(ego_speed=20.0, speed_limit_mps=None)
    obs.step(snap)
    e = next(e for e in obs.history[-1] if e.rule_id == "3r0")
...
def test_vru_collision_when_pedestrian_under_wheel():
    ...
    obs.step(snap)
    e = next(e for e in obs.history[-1] if e.rule_id == "10r0")
    assert e.applies
    assert e.is_violated
...
def test_vru_collision_not_violated_at_safe_distance():
    ...
    obs.step(snap)
    e = next(e for e in obs.history[-1] if e.rule_id == "10r0")
    # Within search radius (30m): applicable but not violated.
    assert e.applies
...
def test_vehicle_collision_triggers_on_overlap():
    ...
    obs.step(snap)
    e = next(e for e in obs.history[-1] if e.rule_id == "9r0")
...
def test_lateral_acceleration_comfort_violation():
    ...
    obs.step(snap)
    e = next(e for e in obs.history[-1] if e.rule_id == "1r11")
    assert e.applies and e.is_violated
...
def test_safe_headway_violation():
    ...
    obs.step(snap)
    e = next(e for e in obs.history[-1] if e.rule_id == "3r3")
    assert e.applies
    # Gap ~ 6 - (4.7 + 4.5)/2 ≈ 1.4 m, ego at 10 m/s → THW well below 1.5 s.
    assert e.is_violated
...
def test_traffic_light_red_in_intersection():
    ...
    obs = TrajectoryObserver()
    obs.step(snap)
    e = next(e for e in obs.history[-1] if e.rule_id == "7r1")
    assert e.applies
    assert e.is_violated


def test_mandatory_stop_rolling_through():
    """Ego still moving as it crosses a stop polygon."""
    ...
    obs = TrajectoryObserver()
    obs.step(snap)
    e = next(e for e in obs.history[-1] if e.rule_id == "8r0")
    assert e.applies
    assert e.is_violated
...
def test_window_summary_integration():
    obs = TrajectoryObserver()
    # 11 ticks at 0.1s, all violating 3r0 with overshoot 5 m/s.
    for k in range(11):
        snap = build_straight_road_scene(
            ego_speed=13.4 + 1.0 + 5.0,  # tolerance + overshoot
    ...
    # Per-step rate is 5^2 = 25; dt = 0.1s; 11 ticks * 25 * 0.1 = 27.5.
...
def test_window_subset():
    obs = TrajectoryObserver()
    for k in range(10):
        # First five over speed limit, last five compliant.
        speed = 25.0 if k < 5 else 12.0
        snap = build_straight_road_scene(ego_speed=speed, timestamp_us=k * 100_000)
        obs.step(snap)
    full = obs.summary()
    early = obs.summary(window_s=(0.0, 0.5))
    late = obs.summary(window_s=(0.5, 1.0))
...
def test_sidewalk_drive_when_on_walkway():
    ...
    # Place ego center on the walkway.
    snap = build_straight_road_scene(ego_y=5.0, ego_speed=2.0, walkways=[walkway])
    obs = TrajectoryObserver()
    obs.step(snap)
    e = next(e for e in obs.history[-1] if e.rule_id == "7r5")
    assert e.applies and e.is_violated
...
def test_bike_lane_encroachment():
    ...
    obs = TrajectoryObserver()
    obs.step(snap)
    e = next(e for e in obs.history[-1] if e.rule_id == "10r5")
    assert e.applies and e.is_violated
...
def test_current_applicable_rules_filters_to_active_set():
    ...
    snap = build_straight_road_scene(ego_speed=10.0)
    obs.step(snap)
    applicable = obs.current_applicable_rules()
    # At minimum, speed limit (lane has one) and comfort rules will be applicable.
